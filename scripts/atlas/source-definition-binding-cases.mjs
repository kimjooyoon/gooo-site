...
setSearch('language-semantic-model');findButton(uiElements.get('content'),'language-semantic-model').click();if(!uiElements.get('detail').textContent.includes('thirteen sources')||!uiElements.get('detail').textContent.includes('DIFFERENT_GLOBAL_OR_BINDING_GRAIN'))fail('semantic component prose reconciliation is not visible');
...
setSearch('language-package-runtime');findButton(uiElements.get('content'),'language-package-runtime').click();if(!uiElements.get('detail').textContent.includes('Go function bodies는 실행하지 않습니다.'))fail('package runtime non-execution boundary is not visible');
