...
if(catalog.obligations?.length!==24||catalog.assurance_obligations?.length!==12||catalog.assurance_obligations.some(obligation=>!assuranceOperationIDs.has(obligation.required_meta_operation)))fail('assurance required_meta_operation does not resolve in the separate assurance_operations registry');
...
console.log(JSON.stringify({schema:'gooo/source-definition-binding-cases/v3',preserved_existing_source_call_count:593,preserved_existing_source_unique_metric_id_count:574,metric_rows:1094,lossless_existing_callsite_multiset:'PASS',multiple_definition_metric_count:11,multiple_definition_callsite_count:30,partial_contracts_not_promoted:97,dynamic_prefix_not_promoted:dynamicRows.length,candidate_rows_with_no_exact_binding:candidateUnboundRows.length,metric_program_operations:9,assurance_operations:14,assurance_required_operation_joins:12,registry_cohorts:'PASS',native_boundary:'PRESERVED_NOT_OBSERVED',existing_cohorts:'PASS',binding_scope:'SOURCE_DEFINITION_ONLY_NOT_RUNTIME_EVIDENCE',new_definitions:0,new_completion_claim:false}));
