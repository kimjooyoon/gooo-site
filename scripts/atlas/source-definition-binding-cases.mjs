...
if(!uiElements.get('detail').textContent.includes('examples/activity-cardinality-resolution/main.gooo:29')||!uiElements.get('detail').textContent.includes('examples/metric-meta-program/main.gooo:20'))fail('one-to-many lexical activity source references were not all preserved');
...
if(!uiElements.get('detail').textContent.includes('동일 이름 lexical activity 없음. name-match source navigation의 빈 상태'))fail('zero-match assurance lexical activity state was not rendered');
