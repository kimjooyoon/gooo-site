...
if(!rendered.includes('typedSourcePolicyDetail(panel,m)')||!rendered.includes('typed source-policy lineage')||!rendered.includes('Indicator·Action identity와 source 경계'))fail('typed source-policy metric detail projection is missing');
class TestElement{
...
if(!sourcePolicyDetail.textContent.includes('typed source-policy lineage')||!sourcePolicyDetail.textContent.includes('Go 파일별 줄 수를 다루는 source-policy 차원')||!sourcePolicyDetail.textContent.includes('observation.Subject')||!sourcePolicyDetail.textContent.includes('meta-observer')||!sourcePolicyDetail.textContent.includes('full Indicator ID')||!sourcePolicyDetail.textContent.includes('MetricID 직접 binding 아님'))fail('typed source-policy metric detail did not preserve contextual fields and boundaries');
const sourcePolicyBase='https://github.com/'+catalog.source_repository+'/blob/'+catalog.source_sha+'/';
const expectedSourcePolicyHrefs=[sourcePolicyBase+'internal/meta/sourcepolicy/vocabulary_part02.go#L13',sourcePolicyBase+'internal/meta/sourcepolicy/evaluate_part02.go#L39',sourcePolicyBase+'internal/meta/sourcepolicy/evaluate_part01.go#L53',sourcePolicyBase+'internal/meta/generation/action.go#L9',sourcePolicyBase+'internal/meta/generation/canonical_part01.go#L25',sourcePolicyBase+'internal/meta/transformationeffect/input.go#L72',sourcePolicyBase+'internal/meta/artifactcoverage/bindings.go#L3'];
