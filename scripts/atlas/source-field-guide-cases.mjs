...
const basisPointsParameters=[{names:['id','class','proof','operation'],type_expression:'string',variadic:false},{names:['satisfied','basisPoints'],type_expression:'bool',variadic:false}];
...
if(sourceAuthorityContracts.length!==3||new Set(sourceAuthorityContracts.map(record=>record.call.line)).size!==3||sourceAuthorityContracts.some(record=>{
  ...
  const signature=record.helper_signature;
  return record.call.kind!=='indicator_constructor_call'||record.helper?.path!=='internal/meta/languageassurance/sourceauthoritypromotion/indicators.go'||record.helper?.line!==29||record.helper?.kind!=='indicator_constructor_definition'||record.metric_id!==expected[0]||record.argument_expressions?.basisPoints!=='true'||record.argument_expressions?.satisfied!==expected[1]||record.result_field_expressions?.Unit!=='unit'||record.result_field_expressions?.Value!=='value'||signature?.type_source!=='GO_AST_TYPE_EXPRESSION_ONLY_NOT_GO_TYPES'||!signature.declaration.includes('func indicator(')||JSON.stringify(signature.parameters)!==JSON.stringify(basisPointsParameters);
}))fail('sourceauthoritypromotion basisPoints source callsite/helper signature is missing, duplicated, or changed');
console.log(JSON.stringify({schema:'gooo/source-field-guide-cases/v4',argument_keys:37,result_keys:32,argument_guide_missing:0,result_guide_missing:0,unknown_new_fields:['constructor','toString','__proto__'],unknown_new_field_status:'UNEXPLAINED',expression_preservation:'PASS',basis_points_source_contract_calls:3,basis_points_helper:'internal/meta/languageassurance/sourceauthoritypromotion/indicators.go:29',basis_points_callsites:[20,21,22],basis_points_parameter_types:'satisfied:bool,basisPoints:bool'}));
