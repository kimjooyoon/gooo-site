...
- CI reuses the pinned upstream checkout to validate every cohort source path, exact line, and declaration anchor from `git cat-file blob <pinned-sha>:<path>`. `metric_translation_cohort.source_backed_count` is the number of records that pass this validation, and `source_reference_validation` records the fixed head and validated reference count. The renderer requires that source checkout; without it, source-backed rendering is rejected. Missing paths, out-of-range lines, wrong anchors, or a different source head/root are explicit `FAIL_CLOSED` counterexamples; an anchor match does not prove the whole Korean sentence or runtime success.
