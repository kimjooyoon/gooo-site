...
const translationCohort={schema:'gooo/source-metric-atlas-translation-cohort/v1',total:untranslatedCohortIds.length,ids:untranslatedCohortIds,source_backed_count:traceability.filter(record=>record.source_evidence?.length).length,semantic_unknown_count:traceability.filter(record=>record.meaning_status?.endsWith('_UNKNOWN')).length,identifier_kind_counts:Object.fromEntries([...new Set(traceability.map(record=>record.identifier_kind))].sort().map(kind=>[kind,traceability.filter(record=>record.identifier_kind===kind).length])),classification_counts:traceabilityCounts,scope:'AUDIT_COHORT_NOT_COMPLETENESS_DENOMINATOR'};
