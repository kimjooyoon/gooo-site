...
function sourceDefinitionBinding(metric,calls){
  ...
  return {schema:'gooo/source-definition-binding/v1',binding_scope:'SOURCE_DEFINITION_ONLY_NOT_RUNTIME_EVIDENCE',state,metric_id:metric.id,exact_metric_id:calls.length?metric.id:null,callsite_count:calls.length,definition_count:definitionCount,callsites:calls,reason,next_operation:nextOperation};
}
...
for(const metric of atlas.metrics){const calls=(sourceCallsByMetricID.get(metric.id)??[]).map(guidedContract);metric.source_contracts=calls;metric.source_definition_binding=sourceDefinitionBinding(metric,calls);}
