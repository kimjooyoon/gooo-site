...
import {metricTranslationCatalog,toolchainSemanticTranslationCatalog} from './metric-translation-catalog.mjs';
export {metricTranslationCatalog,toolchainSemanticTranslationCatalog,toolchainSemanticTranslationCohortIds,untranslatedCohortIds} from './metric-translation-catalog.mjs';
...
export function translateMetric(id){const tokens=id.replace(/^gooo\.metric\./,'').replace(/\.v\d+$/,'').split(/[.:-]+/);const missing=[...new Set(tokens.filter(t=>!terms[t]&&!/^\d+$/.test(t)))];const semanticSourceBacked=toolchainSemanticTranslationCatalog[id];const sourceBacked=metricTranslationCatalog[id]??semanticSourceBacked;if(!sourceBacked)return{ko:tokens.map(t=>terms[t]??t).join(' · '),untranslated_tokens:missing,method:'GLOSSARY_LABEL_NOT_FULL_SEMANTIC_TRANSLATION'};return{ko:sourceBacked.title,untranslated_tokens:semanticSourceBacked?[]:missing,method:semanticSourceBacked?'SOURCE_BACKED_SEMANTIC_TRANSLATION':'SOURCE_BACKED_IDENTIFIER_TRANSLATION',source_backed:sourceBacked};}
